...
- **Executive curation pass** (single-brain, end of operational refresh): **canon council** (`.brain/canon-council.json`), **review debt** (`.brain/review-debt.json`), **executive trust summary** (`.brain/executive-trust-summary.json`), **canon fragility index** (`.brain/canon-fragility.json`), **QoQ diff** (`outputs/reviews/quarter-diff-*.md`), **decision sunset** (`.brain/decision-sunset.json`), **strategic themes** (`.brain/strategic-themes.json` + optional `wiki/work/strategic-themes.md`), **confidence history** (`.brain/confidence-history.json`), **human overrides** (`.brain/human-overrides.json`), **canon admission** (`.brain/canon-admission.json`), **review plans** (`outputs/reviews/review-plan-*.md`), **annual review** (`outputs/reviews/annual-review-*.md`). Dashboard: **`/executive-trust`**, **`/canon-fragility`**, **`/canon-council`**, **`/decision-sunset`**, **`/strategic-themes`**, **`/qoq-diff`**, **`/human-overrides`**, **`/canon-admission`**, plus Home / Executive / wiki confidence panel. See **`docs/GOVERNANCE.md`** for cadence and CLI (`brain executive-trust`, `brain canon-fragility`, `brain canon-council`, `brain review-debt`, …).
...
| `brain canon-guard …` | Off-dashboard trust scan; flags include **`--hook`**, **`--hook --push`** (for git hooks), **`--no-respect-ignore`**, **`--verbose-ignored`**, **`--staged-only`**, **`--json`**, **`--no-save`**. Writes **`.brain/last-canon-guard.json`**. See **`docs/GOVERNANCE.md`**. |
| `brain install-hooks [--pre-commit] [--pre-push] [--all]` | Git hooks: default **pre-commit** only; **`--pre-push`** or **`--all`** adds a **staged-only** scan before **`git push`**. |
...
| `brain executive-trust [--md] [--json]` | Regenerate executive trust + canon fragility JSON (optional markdown report / stdout JSON) |
| `brain executive-trust --ack <actionKey> [--path <wiki>] [--note …]` | Append a voluntary “I did this” row to `.brain/governance-action-log.json` for executive-action telemetry (no refresh) |
| `brain canon-fragility [--refresh] [--limit n]` | TSV from fragility index; `--refresh` rebuilds JSON |
...
- **Home**: trust banner, **executive trust** card when `executive-trust-summary.json` exists, **readiness (doctor, cached)** from **`.brain/last-doctor.json`** (no full doctor re-run on each refresh), plus staleness/trust hints vs current git/state; Obsidian vault name hint; weekly workflow; metrics and actions as before; quick links to trust tools  
- **Executive trust** (`/executive-trust`) and **Canon fragility** (`/canon-fragility`) — compact governance posture and brittle-trusted-page index  
...
| `.brain/executive-trust-summary.json` | Compressed trust posture + domains + wins + actions (weekly control panel inputs); `GET /api/executive-trust` merges **action telemetry** (suggested vs marked-done in a rolling window) from `.brain/governance-action-log.json`. |
| `.brain/canon-fragility.json` | High-trust pages scored as brittle — prioritize before canon expansion. |
